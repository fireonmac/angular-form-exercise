...
export function duplicatedFieldInNeighborFormValidator(formArray: FormArray, fieldName: string) {
  return (control: AbstractControl): ValidationErrors | null => {
    // * 주의사항: 업데이트하고 다시 평가하는 과정에서 무한 루프가 일어날 수 있다
    ...
    // 6. 현재 필드 값이 없는 경우에는 바로 리턴을 함, 따라서 한 글자만 입력했을 때 중복이 일어나면 4번의 2개 -> 1개 문제가 다시 일어남 (v)
    ...
    const namesCountObj = formArray.controls.reduce((acc: any, form, idx: number) => {
      ...
      const name = fControl.value.trim() as string;

      // 현재 수정 중인 인풋은 control과 fControl의 값이 달라서 미리 넣어주고 루프에서는 건너뛴다
      if (idx === 0) {
        acc[currentValue] = 1;
      }
    ...
    formArray.controls.forEach(form => {
      const fControl = form.get(fieldName) as FormControl;
      const fValue = fControl.value.trim() as string;
